...
const Sidebar = () => {
  ...
  const normalLink = 'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-md text-gray-700 dark:text-gray-200 dark:hover:text-black hover:bg-lightGray m-2';
          ...
          <div className="mt-10 ">
            {links.map((item) => (
              <div key={item.title}>
                ...
                {item.links.map((link) => (
                  <NavLink
                    to={`/dashboard/${link.name === 'Dashboard' ? "" : link.name}`}
                    key={link.name}
